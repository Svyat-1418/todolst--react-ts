...
test('correct task should be deleted from correct array', () => {
	...
	const action = taskThunks.removeTask.fulfilled(payload, 'requestId', payload)
...
test('correct task should be added to correct array', () => {
	...
	const action = taskThunks.addTask.fulfilled({ task }, 'requestId', {
		title: task.title,
...
test('title of specified task should be changed', () => {
	...
	const newTaskTitle = 'PostgreSQL'
	const domainModel: UpdateTaskDomainModelType = { title: newTaskTitle }
...
test('empty arrays should be added when correct todolists will be set', () => {
	...
	const endState = tasksReducer(
		{},
		todolistThunks.fetchTodolists.fulfilled(
			{ todolists: correctTodolists },
			'requestId',
			undefined
		)
	)
...
test('property with id should be deleted', () => {
	const action = todolistThunks.removeTodolist.fulfilled({ id: 'todolistId2' }, 'requestId', {
		id: 'todolistId2',
	})
